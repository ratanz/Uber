...
const ConfirmRidePopup = (props) => {
    return (
        ...
        <div className='flex flex-col justify-between items-center gap-3'>
            ...
            <div className='button-container flex md:flex-row justify-center w-full gap-2 p-4 rounded-xl'>
                <Link to='/captain-riding' onClick={() => {
                    props.setConfirmRidePopupPanel(true)
                    props.setRidePopupPanel(false)
                 }}
                    className='bg-green-500 text-center w-full  font-semibold text-white px-10 py-2 rounded-lg'>Confirm</Link>

                <button onClick={() => {
                    props.setRidePopupPanel(false)
                    props.setConfirmRidePopupPanel(false)
                 }}
                    className='bg-red-500 mt- font-semibold  w-full text-white px-10 py-2 rounded-lg'>Cancel</button> 
            </div>
        </div>
